...
> **职责**：将通用 IA 原则（`references/common-ia.md`）映射到 devpace 项目结构（`references/project-structure.md`）的具体规则。
...
六层架构与信息类型映射：详见 `references/project-structure.md`。
...
加载策略详见 `references/project-structure.md`。
