# devpace 设计方案

> **职责**：完整设计方案的永久版本。记录设计理念、概念模型、UX 原则和实现细节。

## 一、设计背景

### 问题

Claude Code 是会话级工具，缺乏跨会话的流程状态感知。要让它融入产品迭代研发流程，需要一套协作机制。

### 设计基础

BizDevOps（必致）白皮书 2022。核心启示：**概念模型是一切的基础**——没有统一的概念模型，就无法对齐目标、打通链路、产生可用数据、持续改进。

### 设计约束

- Claude 自治为主（技术闭环自主推进，关键门禁等待人类审批）
- 度量聚焦质量保障 + 业务价值对齐
- 状态文件使用 Markdown（消费者是 LLM + 人类，不是传统解析器）
- 以 Claude Code Plugin 形态呈现

## 二、UX 设计原则

| # | 原则 | 含义 | 违反示例 |
|---|------|------|---------|
| P1 | **零摩擦接入** | 用户说自然语言就能工作 | 要求先创建 CR yaml 才能编码 |
| P2 | **渐进式暴露** | 日常只看 1 行摘要 | 每次会话输出 50 行状态 |
| P3 | **副产物非前置** | 结构化数据是工作的自动产出 | 要求填写 BR→PF→CR 后才能开始 |
| P4 | **容错恢复** | 任何时刻中断无缝继续 | 中断导致状态不一致 |
| P5 | **自由探索** | 默认自由，正式推进时才走流程 | "CR 被阻塞不能操作" |
| P6 | **分级输出** | 默认简洁，按需展开 | 每次输出门禁状态和度量 |
| P7 | **Git 为源** | 不重复记录 Git 已有信息 | 在 CR 中维护 commit 列表 |

### 7 个 UX 问题及解决方案

1. **认知负担**：用户不需要学 BizDevOps 术语，Claude 自动映射自然语言到作业对象
2. **启动延迟**：三层加载——默认只读 state.md（≤15 行），其余按需
3. **ID vs 自然语言**：用户永远不需要知道 CR-001 的存在
4. **中断恢复**：原子写入——每完成一步就 commit + 更新状态，幂等门禁
5. **自由度**：探索/推进双模式，默认自由探索，修改代码时才进入流程
6. **信息过载**：三级输出（1 句话 / 3-5 行 / 完整状态）
7. **Git 重复**：CR 只记分支名，不记 commit hash

## 三、BizDevOps 概念模型

### 三类核心概念

| 概念 | 白皮书定义 | 在本 Plugin 中的体现 |
|------|----------|-------------------|
| **作业对象** | 价值交付链路上的基本单元 | BR（业务需求）→ PF（产品功能）→ CR（变更请求） |
| **作业空间** | 角色协作的功能区域 | 产品线、交付团队、应用、发布单元 |
| **作业规则** | 流程、规范、约束 | workflow.md（状态机）+ gates.md（门禁） |

### 三个反馈闭环

| 闭环 | 作业对象 | 主导方 | Claude 行为 |
|------|---------|--------|------------|
| 业务闭环 | 业务需求 | 人类 | 辅助分析，不做决策 |
| 产品闭环 | 产品功能 | 人机协同 | 辅助拆解、建议排期、报告进度 |
| 技术闭环 | 变更请求 | **Claude 自治** | 自主编码→测试→验证→停在 review |

### 变更请求状态机

```
created → developing → verifying → review_ready → approved → merged
              ↑            │              │
              └────────────┘              │（人类打回）
              └───────────────────────────┘
```

| 阶段 | 执行者 | 准出条件 |
|------|--------|---------|
| developing | Claude 自治 | 代码提交 + 项目门禁通过 |
| verifying | Claude 自治 | 测试通过 + 项目门禁通过 |
| review_ready | **人类审批** | human review approved |
| approved | Claude 自治 | 合并到主分支 |

## 四、格式决策：Markdown

在 Claude Code 场景中，状态文件的消费者是 LLM + 人类。

| 维度 | Markdown | YAML |
|------|----------|------|
| LLM 解析 | ✅ 同等准确 | ✅ 同等准确 |
| 人类可读性 | ✅ 直接阅读 | ⚠️ 需理解缩进语义 |
| 表达状态 | ✅ checkbox `[x]/[ ]` | ⚠️ `pass/null/fail` |
| Git diff | ✅ 行级差异清晰 | ⚠️ 上下文依赖缩进 |

结论：协作状态、作业规则、度量数据全部使用 Markdown。

## 五、Plugin 结构

```
devpace/                        # Plugin 本体（可分发）
├── .claude-plugin/plugin.json     # 入口声明
├── rules/bizdevops-protocol.md    # 自动注入的行为协议
├── skills/pace-*/SKILL.md          # 5 个 Slash Commands
└── knowledge/                     # 契约 + 模板
    ├── _schema/                   #   格式契约
    └── _templates/                #   空模板

target-project/                    # 使用此 Plugin 的项目
└── bizdevops/                     # /pace-init 生成的项目状态
    ├── state.md                   #   快照（≤15 行）
    ├── product-line.md            #   业务全景 + 价值追溯树
    ├── backlog/CR-*.md            #   变更请求
    ├── iterations/current.md      #   迭代计划
    ├── rules/                     #   项目特有规则覆盖
    └── metrics/dashboard.md       #   度量仪表盘
```

## 六、需求变更管理（核心差异化能力）

### 设计原则

BizDevOps 白皮书："专题更强调拥抱不确定性...在过程中持续迭代、反馈和调整。"

**变更不是异常，是常态。** devpace 把变更当作一等公民——这是与纯任务管理工具的核心差异。

### 四种变更场景

| 场景 | 用户意图 | 系统动作 |
|------|---------|---------|
| 插入新需求 | "还要加一个 X" | 创建 PF+CR，评估容量，建议排期 |
| 暂停/砍掉 | "X 先不做了" | CR→paused，解除依赖，追溯树标 ⏸️ |
| 优先级调整 | "X 优先做" | 重排迭代计划，更新 state.md |
| 修改已有需求 | "X 还要支持 Y" | 评估返工，重置门禁 |

### 处理流程

```
用户表达变更意图
  → Claude 识别变更类型（自然语言触发词）
  → 影响分析（哪些 PF/CR 受影响、依赖链、MoS 影响）
  → 提出调整方案（不自作主张）
  → 等待用户确认
  → 执行更新（CR + 追溯树 + 迭代计划 + state）
  → 记录变更事件到迭代文件
```

### CR 状态机扩展

增加 `paused` 状态：
- 任何状态 → paused（暂停，保留工作成果）
- paused → 暂停前状态（恢复）

### 追溯树变更标记

```
⏸️ — 暂停       🔄 — 进行中     ✅ — 完成
⏳ — 待开始     (新增 MM-DD)    (提前)
```

### 度量扩展

迭代回顾中增加"变更偏差分析"：
- 迭代内发生了几次变更
- 变更导致多少 PF 延后/取消
- 计划 vs 实际的偏差率

## 七、度量体系

基于 BizDevOps 白皮书的 DIKW 模型：

### 质量保障指标

| 指标 | 计算方式 | 用途 |
|------|---------|------|
| 门禁一次通过率 | 首次即通过的 CR / 总 CR | 反映代码质量趋势 |
| 人类打回率 | 打回次数 / 总 review 次数 | 反映设计质量 |
| 缺陷逃逸率 | merged 后缺陷数 | 反映验证充分性 |

### 业务价值对齐指标

| 指标 | 计算方式 | 用途 |
|------|---------|------|
| 成效指标达成率 | MoS 已满足项 / 总项 | 目标对齐度 |
| 需求交付周期 | PF 创建到所有 CR merged | 交付效能 |
| 价值链完整率 | 有完整 BR→PF→CR 链路的占比 | 可追溯性健康度 |

### 度量应用时机

- 每次会话：不输出度量
- 用户问"质量怎样"：质量指标子集
- `/pace-retro`：完整度量 + 回顾报告
- Step 完成时：MoS 评估

## 七、从 diagnostic-agent-framework 迁移的实践

| 实践 | 来源 | 在本项目的应用 |
|------|------|--------------|
| §0 速查卡片 | 每个 rules 文件 | 所有 rules/ 和 _schema/ 文件开头 |
| `> 职责` blockquote | 文件定位标准 | 每个文件首行说清职责 |
| 表格优先 | 规则表达方式 | 规则、对比、分类用表格 |
| _templates/ + _schema/ | 框架模板机制 | 模板供 /pace-init 用，契约供 Claude 参考 |
| plugin.json 同步 | packages-development.md | 文件变更必须同步 plugin.json |
| 质量检查清单 | framework-discipline.md | 提交前检查清单思维 |
