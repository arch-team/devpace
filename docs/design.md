...
**定位**：docs/ 体系的**核心设计文件**——回答"怎么做"。将 vision.md 的目标和 bizdevops-theory.md 的理论转化为可实现的技术方案。

| 维度 | 说明 |
|------|------|
| 回答的问题 | 概念模型怎么映射？UX 怎么设计？状态机怎么流转？文件格式怎么选？变更怎么管理？度量怎么算？ |
| 上游输入 | vision.md（愿景约束设计）、bizdevops-theory.md（理论指导模型选择） |
| 下游消费 | requirements.md（从设计拆解出功能需求）、roadmap.md（设计复杂度影响排期）、model-panorama.md（设计的可视化展开） |
