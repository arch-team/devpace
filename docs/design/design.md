...
- **格式契约**：`knowledge/_schema/auxiliary/risk-format.md`
...
**PF 溢出模式（Overflow Pattern）**：当 PF 信息量增长到一定规模（功能规格 >15 行 | 关联 3+ CR | 经历过 modify），自动溢出为独立文件 `features/PF-xxx.md`。project.md 保留树视图 + `[详情]` 链接。溢出是单向、零摩擦的——对齐 P1 零摩擦、P2 渐进暴露、P3 副产物非前置原则。类比 CR 从第一天就有独立文件（因信息量和生命周期复杂度），PF 在信息丰富后也值得独立存储。格式契约见 `knowledge/_schema/entity/pf-format.md`。
...
字段定义见 `_schema/entity/cr-format.md`。核心约束：
...
**命名规则**：`gate1-passed`、`gate2-passed`、`gate3-approved`。字段定义见 `_schema/entity/cr-format.md`。
...
- 格式定义见 `knowledge/_schema/process/checks-format.md`
...
paused 的状态值和字段定义见 `_schema/entity/cr-format.md`，运行时操作规则见 `skills/pace-init/templates/workflow.md`。
...
- **格式契约**：`knowledge/_schema/process/release-format.md`、`knowledge/_schema/integration/integrations-format.md`
...
- CR type/severity 字段：`knowledge/_schema/entity/cr-format.md`
- 集成配置：`knowledge/_schema/integration/integrations-format.md`（可选，提供告警映射自动化）
...
- **格式契约**：`knowledge/_schema/integration/integrations-format.md`
