...
每个提取的实体按 NEW/DUPLICATE/ENRICHMENT/CONFLICT 分类。合并计划包含来源交叉引用——每项标注来源（文件+行号）、相关已有实体、重复项相似度百分比、丰富项前后对比——帮助做出知情的 accept/reject 决策。import 在 OPP/Epic/BR/PF 层面操作——不创建 CR。
