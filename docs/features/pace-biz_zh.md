...
4. **预览确认** — 变更以 diff 风格预览展示，附就绪度变化对比（旧值 → 新值），确认后才写入
5. **动态下一步推荐** — 基于精炼后就绪度动态推荐：>= 80% 建议进入开发，60-79% 建议可选继续精炼，同 BR 下其他 PF 就绪度较低时也会提醒
6. **建设性退出** — 用户跳过所有维度时，Claude 提供建设性引导（查看全景了解上下文、在开发中自然补充细节、稍后再回来精炼）而非简单的"无变更"信息
...
**覆盖率摘要** — 在统计段末尾展示价值链各层的分解覆盖率（OBJ→Epic、Epic→BR、BR→PF、PF→CR），突出显示存在空白的层级（如"Epic→BR 分解率：3/4（75%，EPIC-003 待分解）"）。
...
4. **验证确认**（1 轮）——轻量去重检查标记与已有实体重叠度 > 70% 的候选，然后审查结构化候选树（OPP→Epic→BR→PF），调整后写入
