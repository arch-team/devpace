...
2. /pace-sync                   → 智能同步：检测变更，呈现摘要，确认后同步全部实体类型
   /pace-sync --dry-run         → 预览变更但不执行
3. /pace-sync link CR-003 #42   → 关联 CR-003 与 GitHub Issue #42
   /pace-sync link EPIC-001     → 智能匹配：按标题相似度搜索 Issue
```

配置完成后，sync-push advisory hook 会在状态变更后自动提醒你运行 `/pace-sync`。智能同步通过内容哈希检测变更，仅推送差异部分。
...
**错误处理**：CR 不存在、外部实体不存在 → 提示用户。CR 已有关联 → 确认后覆盖。无标题匹配 → 建议运行 `/pace-sync` 自动创建。

### `sync`（智能同步——默认命令）

自动检测变更，呈现摘要，确认后执行。吸收了原 `push` 和 `create` 功能。

**语法**：`/pace-sync` 或 `/pace-sync sync [--dry-run]`

运行 `skills/pace-sync/scripts/compute-sync-diff.mjs` 检测所有实体变更（Epic/BR/PF/CR），呈现摘要（变更/新建/无变化），用户确认后按层级顺序（Epic→BR→PF→CR）创建未关联实体的 Issue 并推送变更实体的状态更新。使用 `--dry-run` 仅预览不执行。

**输出示例**：
```
同步检测：
- 3 个实体有变更（CR-003 状态→developing, EPIC-001 完成度↑, PF-002 验收更新）
