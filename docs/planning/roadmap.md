...
| Phase 16 | 企业级扩展 | DORA 代理指标 + 跨项目经验复用 + CI/CD 自动感知 | ⏳ 待启动 |
...
| # | 里程碑 | 状态 | 产出 |
|---|--------|------|------|
| M16.1 | DORA 代理指标 | ⏳ 待启动 | /pace-retro DORA 四指标代理值 + 趋势对比 + 基准分级 + metrics.md 映射规则 |
| M16.2 | 跨项目经验复用 MVP | ⏳ 待启动 | insights 导出/导入机制 + 置信度降级 + 偏好过滤 |
| M16.3 | CI/CD 感知 Tier 1+2 | ⏳ 待启动 | CI 工具类型自动检测 + Gate 4 CI 状态查询 + integrations 持久化 |
