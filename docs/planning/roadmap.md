...
| # | 方向 | 来源 | 触发条件 | 涉及 Skill |
|---|------|------|---------|-----------|
| B1 | /pace-biz refine 子命令（BR/PF 级结构化规格精炼） | flow-next /flow-next:interview | 用户反馈"discover 产出太粗，dev 阶段意图检查点负担重" | pace-biz |
| B2 | Ralph 自主模式（过夜自主开发 + fresh context per iteration） | flow-next Ralph Mode | Agent Teams 稳定 + CR 状态机 headless 支持 | pace-dev（新模式） |
