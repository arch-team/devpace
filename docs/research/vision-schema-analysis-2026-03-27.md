...
没有"解决方案方向"，Vision 只定义了"问题空间"而未触及"解决空间"。对于 AI Agent 来说，这意味着从 Vision 到 OBJ 的分解缺少方向性锚定——Claude 知道"为谁解决什么问题"，但不知道"通过什么路径解决"，导致 OBJ 分解时缺乏约束。

**优化建议**：考虑在核心愿景中增加"产品定位/解决方案方向"字段（一句话描述"我们通过什么方式解决这个问题"），作为 Epic 技术路线选择的上游锚点。

#### 问题 B：北极星指标"唯一性"规则过于刚性

vision-format.md:95 规定"每个产品只有一个北极星指标"。这在理论上是正确的（单一聚焦），但在实践中有两个问题：
...
**优化建议**：
- 保持"聚焦一个"的引导原则，但增加"候选北极星"字段（最多 2-3 个），标注"当前选定 | 候选"状态
- 或在北极星指标 section 增加"阶段"字段（如"验证期/增长期/成熟期"），记录指标选择的上下文
...
**优化建议**：不建议在 vision-format.md 中膨胀字段（违反 IA-11 单一职责），而是：
- 将"核心假设"扩展为结构化格式：`假设 + 验证方法 + 验证状态 + 验证结果`
- 竞争格局和用户画像作为 vision.md 的**可选附录 section**（渐进出现）

#### 问题 D：Vision->OBJ 缺少反向索引
...
**对比**：project.md 有"业务目标"索引表（project-format.md:231-241），但 vision.md 没有类似的 OBJ 索引。Vision 知道北极星指标是什么，但不知道谁在为它贡献。

**优化建议**：在 vision-format.md 的"北极星指标" section 增加 OBJ 贡献索引（渐进字段，Claude 自动维护）：
...
| OBJ | 贡献方式 | MoS 进度 |
|-----|---------|---------|
| OBJ-001 | 用户增长 -> MAU 直接贡献 | 2/3 |
| OBJ-003 | 留存优化 -> MAU 间接贡献 | 1/2 |
```

#### 问题 E："渐进丰富"与"战略锚点"的设计张力
...
**优化建议**：区分"文件可以渐进"和"核心字段必须有意义"。具体做法：
- 保持文件整体的渐进丰富设计
- 但在 Schema 中标注"最小有效状态"——Vision 至少需要填充"一句话愿景摘要 + 目标用户 + 核心问题"才算有效（而非只是非空的桩）
- 在 state.md 或 project.md 中增加"vision_effective: true/false"标记，供下游 Skill 判断是否已有有效 Vision
...
| # | 维度 | 建议 | 性质 | 优先级 |
|---|------|------|------|:------:|
| E | Schema 语义 | 区分"桩状态"和"最小有效状态"，定义 Vision 生效的最低标准 | Schema 优化 | 1 |
| D | Schema 结构 | 北极星指标 section 增加 OBJ 贡献索引（渐进，Claude 自动维护） | Schema 优化 | 2 |
| F | 实现补齐 | P0/P1/P2 的 8 项实现断裂修复（第四章） | 实现补齐 | 3（可与 D/E 并行） |
| A | Schema 字段 | 核心愿景增加"产品定位/解决方案方向"字段 | Schema 优化 | 4 |
| C | Schema 字段 | 核心假设扩展为结构化格式（假设+验证方法+状态+结果） | Schema 优化 | 5 |
| B | Schema 字段 | 北极星指标增加"候选"或"阶段"字段，降低早期使用门槛 | Schema 优化 | 6 |

**核心结论**：

> **Schema 是契约，不是实现。** vision-format.md 声明了"谁在什么时候做什么"，但消费方的 procedures 尚未履行契约。修复的核心不是改 Schema，而是让 procedures 履行 Schema 已经定义好的契约——同时在 Schema 层面补齐"最小有效状态"和"OBJ 贡献索引"两个结构性缺失。
