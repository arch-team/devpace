...
统一写入：pace-learn是唯一写入者 | 其他Skill产出学习请求
上下文置信度：全局值 + 按CR类型维度（功能/修复/hotfix）
全局库：~/.devpace/global-insights.md（置信度≥0.8+验证≥3自动同步）
```
...
<!-- 按生命周期分区：Active 在前，Dormant 次之，Archived 在末尾 -->
...
- **冲突对**：[无 | 对方 pattern 标题]
- **上下文置信度**：[功能:X.X | 修复:X.X | hotfix:X.X]（可选，仅多次验证后生成）
- **日期**：[首次记录日期]
...
insights.md 中的 pattern 有三种生命周期状态：
...
- pace-learn 每次写入 insights.md 时顺便检查全部条目状态
- /pace-retro 回顾时输出生命周期健康度
...
置信度支持按 CR 类型维度细分（可选扩展，仅在同一 pattern 被多次不同类型 CR 验证后生成）：

```
- **置信度**：0.7（全局）
- **上下文置信度**：[功能:0.8 | 修复:0.5 | hotfix:0.3]
```
...
满足以下条件的 pattern 自动同步到 `~/.devpace/global-insights.md`：
- 置信度 ≥ 0.8
- 验证次数 ≥ 3
- 类型 ≠ 偏好（偏好是项目特定的）

同步行为：
- pace-learn 写入 insights.md 后检查新写入/更新的 pattern 是否满足同步条件
- 满足 → 追加或更新到全局库，带项目来源标记：`- **来源项目**：[项目名]`
