...
内容：Epic 关联 + OBJ 关联（链接）+ 状态 + 来源 + 优先级 + 业务上下文 + 成效指标（双维度 MoS）+ 关键流程（可选）+ PF 列表
...
> 当需求涉及多步骤操作、条件分支或审批流程时，通过 /pace-biz refine 引导补充。
...
| 关键流程 | 渐进 | /pace-biz refine 检测到流程关键词时引导 | 编号步骤+条件分支+异常路径（可选 section，仅涉及多步骤操作的 BR） |
