...
意图 section 的可选扩展。复杂度自适应：S 不生成 | M 可选 | L/XL 必须。
...
- `hotfix` 可走加速路径（跳过部分门禁，但仍需事后审批记录）
...
| 事件 | 更新字段/section | 说明 |
|------|-----------------|------|
| /pace-dev 创建 CR | 元信息 + 意图（用户原话/范围/验收条件） | Claude 渐进填充；复杂度自适应验收条件格式 |
| 意图检查点完成 | 意图（方案/约束/歧义标记）+ 复杂度 | L/XL 进入 developing 前方案必填；复杂度自动评估 |
| 进入 developing | 执行计划（L/XL 必须）+ 事件表 | S 不生成执行计划；事件表追加 status_change |
| Gate 1 通过/失败 | 质量检查 checkbox + 事件表 | 事件表追加 gate1_pass 或 gate1_fail |
| L/XL 步骤完成 | 执行快照（步骤进度）+ 事件表 | 事件表追加 step_done |
| Gate 2 通过 | 质量检查 checkbox + 事件表 | 所有歧义标记必须已解决 |
| /pace-test impact | 影响分析 section | 多次执行时覆盖更新 |
| /pace-test accept | 验证证据 section | 逐条比对验收条件 |
| /pace-guard scan | 风险预评估 section | L/XL 进入 developing 前触发；S/M 匹配 defense pattern 时触发 |
| 推进模式 checkpoint | 运行时风险表 + 事件表 | Medium/High 同步创建 RISK-NNN.md |
| 审批/打回 | 事件表 | approved/rejected + 打回信息结构化 |
| 状态转换 | 元信息状态 + 事件表 | 每次转换追加 status_change 事件行 |
| 会话结束 | 执行快照（L/XL） | 更新最近 checkpoint 和恢复建议 |

## 容错

| 异常 | 处理 |
|------|------|
| CR 文件丢失 | 从 project.md 价值功能树状态行 + state.md 当前 CR 信息重建最小 CR（元信息+状态）；事件表不可恢复 |
| CR 与 project.md 树视图状态不一致 | 以 CR 文件为准（更详细），同步更新树视图状态标记 |
| 意图 section 缺失或格式异常 | 保持已有字段；缺失的意图字段不阻断流程——最小可用 CR 仅需元信息 |
| 事件表格式损坏 | 保留可解析行，跳过损坏行；度量计算时标注"部分数据" |
| 无 type 字段 | 向后兼容——自动视为 feature |
| 无复杂度字段 | 向后兼容——视为未评估，不影响流程 |
| 无方案字段（S/M） | 向后兼容——S/M 不要求方案字段 |
| 不使用 Release 流程 | 向后兼容——merged 仍为有效终态 |
| 旧格式阻塞字段（仅 blocked_by） | 向后兼容——自动视为 blocks 关联关系 |
| 外部关联字段缺失 | 向后兼容——不影响任何内部流程 |
| 执行快照缺失（L/XL） | 不阻断——按无快照模式推进，会话恢复时重新评估进度 |
