...
内容：OBJ 关联（主/副）+ 状态 + 来源 + 时间框架 + 背景 + 双维度 MoS + 利益相关者（可选）+ BR 列表
...
| 角色 | 关切 | 影响程度 |
|------|------|:--------:|
| [角色名，如"终端用户"] | [核心关切，如"操作效率"] | 高/中/低 |

> 首次创建时可为空，discover/decompose 过程中渐进补充。用于 align 对齐检查和优先级决策参考。
...
| 利益相关者 | 渐进 | discover/decompose 过程中引导 | 角色+关切+影响程度表格（可选，用于 align 检查） |
...
| 依赖 | 依赖的其他 BR 编号（可选，多个用逗号分隔，无依赖时 `—`） |
