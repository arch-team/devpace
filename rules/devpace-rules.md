...
当用户说"帮我做 X"时：
- **未初始化降级**：`.devpace/` 目录不存在 → 以探索模式直接协助完成请求（不创建状态文件），完成后自然提及"如果想持续追踪开发进度，可运行 /pace-init"
- `.devpace/backlog/` 中无对应 CR 时：
...
1. **智能同步**：`/pace-sync`（无参数）自动检测全部实体（Epic/BR/PF/CR）变更，呈现摘要后用户确认执行。增量检测基于内容哈希（脚本 `skills/pace-sync/scripts/compute-sync-diff.mjs` 计算）
