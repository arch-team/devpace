...
- **Pre-flight**：L/XL CR 意图检查点自动触发风险预扫描——5 维评估（历史教训/依赖影响/架构兼容性/范围复杂度/安全敏感度），结果写入 CR "风险预评估" section。S/M CR 仅在 insights.md 有匹配 defense pattern 时触发（详见 `skills/pace-dev/dev-procedures-intent.md`，权威源）
