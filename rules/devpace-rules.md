...
### 可选功能（§14）
Release：`.devpace/releases/` 存在时生效 | 两层渐进暴露：用户层（create/deploy/verify/close/full/status + 空参引导向导）· 专家层（changelog/version/tag/notes/branch/rollback）| close 自动包含 changelog+version+tag | Gate 4 发布门禁（依赖 integrations/config.md） | Release 状态机 staging→deployed→verified→closed（+rolled_back 分支）| 环境晋升（多环境渐进部署）
...
- L/XL CR 执行计划生成后，执行计划反思（4 维度：需求覆盖 / 过度设计 / 拆分必要性 / 假设合理性）——详见 `dev-procedures.md`
- Gate 1/2 通过后，附加 1-2 行轻量反思观察（技术债 / 测试覆盖 / 边界场景 / 验收全面性），记录到 CR 事件表——详见 `dev-procedures.md`
...
- developing 阶段：聚焦实现和 Gate 1，不提前考虑 Gate 2 或 Review 策略
- verifying 阶段：聚焦质量验证和 Gate 2，不提前考虑合并或连锁更新
- 目的：防止 AI 在长上下文中"偷看"后续步骤而跳过或合并当前步骤
- 预防合理化："我已经知道下一步了"→知道不等于应该提前做 | "一起做更高效"→Gate 存在的意义就是阶段门控 | "用户没时间等"→步骤隔离保护质量，不可因时间压力跳过
...
**溯源标记**：创建 CR 和更新 project.md 时，Claude 对新增内容添加 HTML 注释溯源标记（格式见 `project-format.md` 和 `cr-format.md` 溯源标记章节）。标记不影响 Markdown 渲染，日常输出中不展示。
...
- 与渐进教学互斥：同一输出不同时附教学（§17）和推理后缀——教学优先（首次），后续用推理后缀。
...
- **project.md**：BR/PF/CR 行、范围条目、功能规格等新增内容（格式见 `project-format.md` 溯源标记章节）
- **CR 意图 section**：用户原话、范围、验收条件等字段（格式见 `cr-format.md` 溯源标记章节）
...
- **非法状态值**：CR 文件中的状态值不在合法列表（created/developing/verifying/in_review/approved/merged/released/paused）时，提示用户确认正确状态，不自动修正
- **缺失类型字段**：CR 无 `类型` 字段时视为 `feature`（向后兼容 v0.1.0 项目），不自动写入
- **缺失目录**：`releases/`、`integrations/`、`iterations/`、`metrics/` 不存在时不报错，相关功能降级为不可用。当 Skill 需要写入缺失目录时（如 `/pace-plan` 写 `iterations/`、`/pace-retro` 写 `metrics/`），自动创建目录后继续，不提示用户
...
**触发词**：详见 `skills/pace-change/SKILL.md` description 字段。

识别到变更意图时，**先分类再处理，不立即执行变更**。分类和处理流程见 `skills/pace-change/change-procedures.md`。

### 未初始化项目的变更检测

当项目未初始化 `.devpace/`（无 state.md）时，Claude 检测到变更意图触发词后：

1. 仍执行即时影响分析——基于代码库结构（import/require 依赖、模块划分）
2. 输出与正常模式相同格式的影响评估（涉及模块、依赖关系、风险点、建议）
3. 输出末尾附加引导："想要持久化追踪这些变更？运行 /pace-init 开始。"

**不创建 .devpace/ 目录或状态文件**。仅提供单次分析价值，让用户体验到变更管理的差异化能力。

### 与 /pace-change 的关系

§9 在 Claude 自动检测到变更意图时生效。/pace-change 是用户显式调用的 Skill，提供参数化输入和完整交互流程。两者共享同一套处理流程（change-procedures.md）。

## §10 主动节奏管理

Claude 在推进模式中主动监控研发节奏，检测异常信号并输出非打断式建议。不等用户询问，主动提供节奏感知。

### 信号检测与主动行为

| 信号 | 检测方式 | 主动行为（含推理后缀） |
|------|---------|---------|
| CR 在 developing 超过 5 个 checkpoint | 当前 CR 的 checkpoint 计数 | "当前变更已 N 个 checkpoint，涉及 M 个文件——考虑拆分？" |
| 迭代完成率 > 80% | 读 current.md PF 完成率 | "迭代完成 N%（剩余 M 个功能点）——建议规划下一迭代" |
| in_review 积压 > 2 个 | 扫描 backlog/ CR 状态 | "有 N 个变更等待 review——[最关键 CR 名称] 阻塞 [下游功能]" |
| 同一检查项失败 > 2 次 | CR 质量检查 checkbox 历史 | "[检查项] 连续失败 N 次——建议调整检查策略或拆分变更" |
| 本会话已 merged 2+ CR | 会话内计数 | "本会话已合并 N 个变更——建议做一次增量回顾" |
| 迭代变更 > 3 次 | 读 current.md 变更记录条数 | "本迭代已有 N 次范围变更——原始目标可能偏移，注意对齐" |

### 执行规则

- **触发时机**：推进模式中每完成 **5 个 checkpoint** 后，或会话中超过 30 分钟未切换 CR 时，自动调用 `pace-pulse` Skill
- **不打断**：建议以 1-2 行自然语言附加在 checkpoint 输出之后，不阻断工作流
- **不重复**：同一信号在同一会话中只提醒一次
- **会话总量上限**：每会话最多输出 **3 条**节奏提醒（含 §1 会话开始的提醒），达到上限后静默
- **静默优先**：无信号触发时不输出任何内容
...
CR 进入 merged 后，Claude 自动执行以下管道（不需用户指令）：

1. **连锁更新**（已有，§2 定义的 4 步）
2. **即时知识积累**：自动调用 `pace-learn` 提取本 CR 的 pattern → 追加 insights.md
3. **增量度量更新**：更新 dashboard.md 中可增量计算的指标（质量检查一次通过率、变更周期）
4. **Release Note 检查**：如该 CR 所属 PF 的所有 CR 均 merged → 自动生成变更摘要追加到 iterations/current.md
5. **迭代完成度检查**：计算 PF 完成率 → > 90% 时附加建议"迭代接近完成，建议 /pace-retro 后 /pace-plan"
6. **首个 CR merged 回顾**（仅首次，教学标记 `first_merged` 去重）：第一个 CR merged 后，附加 3 行回顾：
...
| 条件 | Claude 行为（含推理后缀） |
|------|------------|
| PF 完成率 = 100% | "迭代 N 个功能全部完成——建议回顾后规划下一迭代" |
| PF 完成率 > 80% | "迭代完成 N%（剩余 M 个功能点）——接近尾声"（§1 已覆盖） |
| 距结束日 < 2 天且完成率 < 50% | "距结束还有 N 天，完成率仅 M%——建议调整范围或延期" |
| 上一迭代关闭 + 无 current.md | "上一迭代已关闭——建议规划新迭代" |

### 功能发现（嵌入式触发）

根据用户使用进度，在自然时机**直接执行**进阶功能（而非提示命令名称）。每条仅出现一次（使用 §17 教学标记去重）。触发条件和行为见 `skills/pace-dev/dev-procedures.md`。
...
| 功能 | 前置条件 | 详细规则 |
|------|---------|---------|
| 发布管理 | `.devpace/releases/` 存在 | `release-procedures.md` |
| 运维反馈 | `.devpace/releases/` 存在 | `feedback-procedures.md` |
| 集成管理 | `.devpace/integrations/config.md` 存在 | `release-procedures.md` |

Release 是可选功能——未配置时 merged 仍是有效终态。集成完全可选，手动摄入始终可用。

### 发布编排能力

devpace 可在用户授权下执行发布动作，通过 git/gh 等标准工具编排（不替代 CI/CD）：

**用户层子命令**（argument-hint 可见，日常使用）：

| 能力 | 子命令 | 前置条件 | 降级行为 |
|------|--------|---------|---------|
| 创建 Release | `/pace-release create` | merged CR 存在 | 始终可用 |
| 记录部署 | `/pace-release deploy` | Release 处于 staging | 始终可用 |
| 执行验证 | `/pace-release verify` | Release 处于 deployed | 始终可用 |
| 完成发布 | `/pace-release close` | Release 处于 verified | 自动包含 changelog+version+tag，各步可跳过 |
| 一键发布 | `/pace-release full` | Release 处于 verified | close 的推荐别名 |
| 查看状态 | `/pace-release status` | Release 存在 | 始终可用 |
| 引导向导 | `/pace-release`（空参数） | 任何状态 | 根据 Release 状态自动引导下一步 |

**专家层子命令**（功能保留，不在 argument-hint 中展示）：

| 能力 | 子命令 | 说明 |
|------|--------|------|
| Changelog 生成 | `/pace-release changelog` | close/full 的自动步骤，也可独立调用 |
| 版本号 Bump | `/pace-release version` | close/full 的自动步骤，也可独立调用 |
| Git Tag 创建 | `/pace-release tag` | close/full 的自动步骤，也可独立调用 |
| Release Notes | `/pace-release notes` | 生成 BR 级 Release Notes，可独立调用 |
| 发布分支 | `/pace-release branch` | Git 工作流专家功能 |
| 回滚追踪 | `/pace-release rollback` | 异常处理，status 会在需要时推荐 |

### 发布分支规则

- **直接发布（默认）**：无发布分支配置时，所有操作在 main 分支完成
- **Release 分支**：create 时可选创建 `release/v{version}`，close 时提示 merge 回 main
- **Release PR**：可选生成包含 changelog + version bump 的 PR，用户 merge PR = 确认发布
- **配置依赖**：`integrations/config.md` 的"发布分支"段（分支模式/前缀/PR/自动合并）

### 环境晋升规则

- **晋升顺序**：integrations/config.md 环境表行序（首行→末行）
- **逐环境部署**：deploy(env1) → verify(env1) → deploy(env2) → verify(env2) → ... → deployed
- **状态机不变**：deployed = 最终环境已部署，staged 期间的多环境部署通过部署记录表追踪
- **降级**：单环境或无配置时，行为与之前完全一致

### Gate 4 系统级发布门禁

Release create 后、deploy 前的系统级检查（可选，依赖 integrations/config.md）：

- 运行构建/测试命令验证代码可构建
- 检查 CI pipeline 状态
- 确认所有纳入 CR 的 Gate 1/2/3 均已通过
- 无配置时 Gate 4 静默跳过
...
- **rolled_back** 是终态，修复后需创建新 Release
- deployed 和 verified 状态转换需人类确认

### Release 关闭连锁更新

Release close 时自动执行（需用户确认前 3 步）：

1. 生成 Changelog（写入 Release 文件 + 用户产品 CHANGELOG.md）
2. Bump 版本号（更新配置的版本文件，无配置则跳过）
3. 创建 Git Tag + 可选 GitHub Release
