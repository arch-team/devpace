...
```
会话开始 → 读 .devpace/state.md → 节奏健康度检测 → 1 句话报告（+节奏提醒）→ 等用户指令
探索模式（默认）→ 自由读取分析，不改 .devpace/（硬约束，退出时校验）
推进模式（改代码时）→ 绑定 CR → 意图检查点 → 走状态机 → checkpoint（含漂移检测）
质量检查不过 → 自行修复，不请示用户
...
4. **节奏健康度检测**（state.md 存在时额外执行）：
   - 读取 `.devpace/metrics/dashboard.md` 的"最近更新"日期，计算距今天数
