...
3. **IR-3** L/XL 不跳过意图检查点（dev-procedures.md）
...
可选：Release/集成/反馈/同步——目录或文件存在时生效（§14, §16）| 渐进教学——首次触发附 1 句（§15）
...
| 系统 | pace-learn · pace-pulse — Claude 自动调用 |
...
- 用户说明确修改意图 + **本会话首次** → Claude 自然确认："我来帮你管理这次开发——跟踪进度、自动检查质量、记录变更。需要这样做，还是只想快速改个东西？"
...
**通用行为**（所有自主级别）：
> 推进模式的详细执行规则（原子步骤、漂移检测、执行计划反思、Gate 反思、隔离规则）见 `skills/pace-dev/dev-procedures.md`（权威源）。
...
**核心行为**：Triage 分类（范围/复杂度/紧急度）→ 影响分析（关联 CR/PF/迭代）→ 决策（执行/暂停/拆分/拒绝）。详细流程见 `skills/pace-change/change-procedures.md`（权威源）。
...
- **Pre-flight**：L/XL CR 意图检查点自动触发风险预扫描——5 维评估（历史教训/依赖影响/架构兼容性/范围复杂度/安全敏感度），结果写入 CR "风险预评估" section。S/M CR 仅在 insights.md 有匹配 defense pattern 时触发（详见 dev-procedures.md，权威源）
- **Runtime**：推进模式 checkpoint 增加轻量风险信号检测（技术债引入/安全隐患/架构腐化 3 类），Medium/High 风险持久化到 `.devpace/risks/`
...
1. **连锁更新**：PF 状态 + project.md 功能树 + state.md + iterations/current.md + Release 纳入判断
...
6. **首个 CR 回顾**（教学标记 `first_merged` 去重）：附 3 行说明
...
根据用户使用进度，在自然时机**直接执行**进阶功能（非提示命令名称）。每条仅 1 次（§15 教学标记去重）。触发条件和行为见 `skills/pace-dev/dev-procedures.md`（权威源）。
...
| 同步管理 | `.devpace/integrations/sync-mapping.md` 存在 | `skills/pace-sync/sync-procedures.md` |
...
- **Release close**：changelog + version + tag + 连锁更新——详见 `skills/pace-release/release-procedures-lifecycle.md` 和 `skills/pace-release/release-procedures-expert.md`（权威源）
...
1. **手动推送为主**（Phase 18 MVP）：CR 状态变化后**提醒**推送（sync-push Hook），不自动执行外部操作
2. **关联管理**：`/pace-sync link` 建立 CR ↔ 外部实体 1:1 映射，记录在 CR 文件和 sync-mapping.md
3. **状态映射**：推送时按 sync-mapping.md 状态映射表翻译 devpace 状态为外部标签/操作
4. **只推不拉**（Phase 18 MVP）：仅支持 devpace → 外部方向，不消费外部状态变化
5. **幂等操作**：重复 push 同一状态不产生副作用（标签已存在则跳过）
6. **降级静默**：gh CLI 不可用时 push 报错并提示安装，不阻断核心工作流
7. **副产物非前置三阶段**（渐进消除手动前置）：
   - pace-init 检测 git remote 时提议同步配置（Step 8）
   - CR 创建时提议创建外部 Issue（sync-mapping.md 存在时，自主级别分化）
   - merged 时自动推送（§11 第 7 步，已实现）

### 与现有规则的协调

- §2 推进模式状态转换后 → sync-push Hook 触发提醒（advisory，不阻断）
- §11 merged 后连锁更新第 7 步 → 自动推送 merged 状态到外部（关闭 Issue + done 标签 + 完成摘要）
