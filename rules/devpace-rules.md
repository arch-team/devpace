...
专项：/pace-release（发布编排：changelog/version/tag/rollback/full/notes/branch） · /pace-feedback · /pace-role · /pace-theory · /pace-trace

### 可选功能（§14）
Release：`.devpace/releases/` 存在时生效 | changelog · version · tag · rollback · full 一键发布 · notes（BR 级 Release Notes）· branch（发布分支管理）· 环境晋升（多环境渐进部署） | Gate 4 发布门禁（依赖 integrations/config.md） | Release 状态机 staging→deployed→verified→closed（+rolled_back 分支）| close 连锁更新（Changelog+版本+Tag+CR 状态+功能树+度量）
...
| 能力 | 子命令 | 前置条件 | 降级行为 |
|------|--------|---------|---------|
| Changelog 生成 | `/pace-release changelog` | Release 存在 | 始终可用（从 CR 元数据生成） |
| 版本号 Bump | `/pace-release version` | integrations/config.md 版本管理段 | 提示用户手动更新版本文件 |
| Git Tag 创建 | `/pace-release tag` | Release 版本号已确定 | 用户手动 git tag |
| GitHub Release | `/pace-release tag` | gh CLI 可用 | 跳过，仅创建 tag |
| 回滚追踪 | `/pace-release rollback` | Release 处于 deployed | 始终可用 |
| 一键发布 | `/pace-release full` | Release 处于 verified | 按步骤执行，各步可跳过 |
| Release Notes | `/pace-release notes` | Release 存在 | 始终可用（从 CR+PF+BR 元数据生成） |
| 发布分支 | `/pace-release branch` | integrations/config.md 发布分支段 | 所有操作在 main 分支完成 |
