...
1. 探索模式不改 `.devpace/`（§2）
2. Gate 3 人类审批不可绕过（§2）
3. L/XL 不跳过意图检查点（dev-procedures.md）
4. 推进模式绑定 CR + 状态机（§2）
5. merged 后执行连锁更新（§11）
...
### 质量与审批
质量检查 = 命令+意图+对抗 → 不通过自修复 → human_review 铁律（Gate 3 不可绕过）
详见 §2 推进行为、§14 Release（Gate 4）

### 自主级别
辅助（建信任）| 标准（默认）| 自主（高信任）— 读 project.md 自主级别字段

### Agent 鲁棒性
fork 不可用 → inline 回退（静默，功能不降级，无 Agent Memory）
执行透明 → 完成后输出变更摘要（改了什么 + 关键决策 + 注意事项），禁止静默写入 .devpace/

### 主动节奏 + 风险
每 5 checkpoint 脉搏检查 | 8 信号（含风险积压）| 每会话最多 3 条提醒 | L/XL 意图检查点风险预扫描
分级响应：Low 静默 | Medium 提醒+方案 | High 暂停等人（铁律）
...
4. **节奏健康度检测**（state.md 存在时额外执行）：读取 dashboard.md + backlog/ + current.md → 按信号优先级附加最高 1 条提醒。信号定义详见 `skills/pace-pulse/pulse-procedures.md` "会话开始节奏检测"章节。无触发时静默
5. 等待用户指令（不自作主张开始工作）
...
> **核心**：每 5 checkpoint 脉搏检查，6 种信号非打断式建议，每会话 ≤3 条。
...
- **Pre-flight**：L/XL CR 意图检查点自动触发风险预扫描——5 维评估（历史教训/依赖影响/架构兼容性/范围复杂度/安全敏感度），结果写入 CR "风险预评估" section。S/M CR 仅在 insights.md 有匹配 defense pattern 时触发（详见 dev-procedures.md）
- **Runtime**：推进模式 checkpoint 增加轻量风险信号检测（技术债引入/安全隐患/架构腐化 3 类），Medium/High 风险持久化到 `.devpace/risks/`
- **脉搏第 8 信号**：open 风险 > 3 或 High 风险 > 0 时触发提醒（详见 pulse-procedures.md）
...
教学内容详见 `knowledge/teaching-catalog.md`。
