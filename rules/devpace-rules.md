...
可选：Release/集成/反馈——目录存在时生效（§14）| 渐进教学——首次触发附 1 句（§15）
...
forked Agent 完成后，用户必须清楚"做了什么、改了什么、为什么"。规则：

- **pace-dev**：实现完成后输出变更摘要——`📋 变更：[N 文件] | 关键决策：[决策列表] | 注意：[风险/遗留]`
- **pace-change**：影响分析完成后输出——`📋 影响范围：[受影响 PF/CR] | 建议动作：[暂停/调整/...] | 变更原因：[来自用户]`
- **pace-plan**：规划完成后输出——`📋 迭代范围：[N 个 PF / M 个 CR] | 预估工作量：[基于历史] | 关键取舍：[纳入/排除原因]`
- **pace-retro**：报告完成后输出——`📋 度量亮点：[关键指标] | 改进建议：[N 条] | 经验提取：[新 pattern]`
- **不透明动作禁令**：Agent 不得静默修改 .devpace/ 文件后不告知用户。任何状态文件写入（CR/state.md/project.md/insights.md）必须在摘要中体现
