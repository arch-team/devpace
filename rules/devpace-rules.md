...
导航：信号优先级 SSOT（signal-priority.md）| next/pulse/status 三 Skill 共享 | session-start 推→next 拉去重 | merged 后回路建议（§11）
...
5. **迭代完成度检查 + 下一步建议**：PF 完成率 >90% → 建议 `/pace-retro`（完整回顾，含迭代传递清单）后 `/pace-plan`。完成度检查后，如有其他域信号（审批积压/风险/Release 待验证等），附加 1 行轻量建议："已处理完 [CR 标题]。→ /pace-next 查看下一步"——引导用户进入跨域导航
...
- **merged 后**（§11 第 5 步）：完成迭代检查后附加 1 行 pace-next 回路建议（见上文）
- **target skill 完成后**：用户执行 /pace-review 或 /pace-release 等目标 Skill 后，如仍有其他域待处理信号，追加"→ /pace-next 查看下一步"
