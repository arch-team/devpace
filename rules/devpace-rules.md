...
### 节奏 + 风险 + 质量
脉搏：每 5 checkpoint | ≤3 条/会话 | 会话结束摘要 | High 暂停等人（§10, §6）
...
4. **节奏健康度检测**（state.md 存在时额外执行）：读取 dashboard.md + backlog/ + current.md → 按信号优先级输出分层摘要（最高 1 条完整建议 + 其余触发信号精简列表，≤3 行）。信号定义详见 `skills/pace-pulse/pulse-procedures.md` "会话开始节奏检测"章节（权威源）。无触发时静默
5. 等待用户指令（不自作主张开始工作）
...
4. **节奏摘要**（state.md 存在时）：输出 1-2 行会话级节奏总结。格式和规则详见 `skills/pace-pulse/pulse-procedures.md` "会话结束节奏摘要"章节（权威源）
...
- **约束**：不打断（附加在 checkpoint 后）| 不重复（同信号同会话仅 1 次，严重度 ×2 升级除外）| 标准配额 ≤3 条（长会话 >2h 放宽至 ≤5 条）| 无信号时静默（正向反馈 ≤1 次/会话不计配额）
