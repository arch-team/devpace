...
PF 溢出：功能规格>15行|3+CR|modify → 自动迁移到 features/PF-xxx.md（§11 连锁更新 + dev-procedures）
...
完成率 100%→建议回顾+规划 | >80%→接近尾声（§1 已覆盖）| 距结束<2 天且<50%→建议调整范围 | 上一迭代关闭+无 current.md→建议规划新迭代 | pace-change add 后迭代容量超出→建议 `/pace-plan adjust` | 迭代健康度低于 0.5→建议 `/pace-plan adjust` 或范围缩减。
