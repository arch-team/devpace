...
专项：/pace-release（发布编排：create/deploy/verify/close/full/status + 空参引导 | 专家：changelog/version/tag/notes/branch/rollback） · /pace-test（测试验证：accept/strategy/coverage/impact[--run]/report[REL-xxx] | 深度：generate[--full]/flaky/dryrun/baseline） · /pace-feedback · /pace-role · /pace-theory · /pace-trace
...
Schema：state→state-format | CR→cr-format+cr-reference | 项目→project-format | 迭代→iteration-format | 检查→checks-format | Release→release-format | 上下文→context-format | 度量→insights-format | 集成→integrations-format
...
**溯源标记**：创建 CR 和更新 project.md 时，Claude 对新增内容添加 HTML 注释溯源标记（格式见 `knowledge/_schema/project-format.md` 和 `knowledge/_schema/cr-format.md` 溯源标记章节）。标记不影响 Markdown 渲染，日常输出中不展示。
...
根据用户使用进度，在自然时机**直接执行**进阶功能（而非提示命令名称）。每条仅出现一次（使用 §15 教学标记去重）。触发条件和行为见 `skills/pace-dev/dev-procedures.md`。
...
| 功能 | 前置条件 | 详细规则 |
|------|---------|---------|
| 发布管理 | `.devpace/releases/` 存在 | `skills/pace-release/release-procedures.md` |
| 运维反馈 | `.devpace/releases/` 存在 | `skills/pace-feedback/feedback-procedures.md` |
| 集成管理 | `.devpace/integrations/config.md` 存在 | `skills/pace-release/release-procedures.md` |
...
子命令表、发布分支规则、环境晋升详情、Gate 4 检查项、关闭连锁更新清单——详见 `skills/pace-release/release-procedures.md`。
...
| AI 验收验证 | 首次执行 /pace-test accept 时 | "（accept 是验证员视角——尝试执行、收集证据；Gate 2 是守门员视角——对照意图判定。两者互补，不重复。）" | `accept` |
