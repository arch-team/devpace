...
| 作业对象 | Schema 文件 | 关键字段/适用场景 |
|---------|------------|-----------------|
| state.md | state-format | 会话锚点，≤15 行，目标+当前+下一步 |
| CR | cr-format + cr-reference | 状态机、意图 section、质量检查 |
| 项目 | project-format | 价值功能树、自主级别、范围 |
| 迭代 | iteration-format | 迭代计划、进度追踪 |
| 检查 | checks-format | Gate 1/2 检查项、命令/意图检查 |
| Release | release-format | 发布状态机、环境晋升、Gate 4 |
| 上下文 | context-format | 技术约定、编码规范 |
| 度量 | insights-format | 经验 pattern、置信度 |
| 集成 | integrations-format | CI/CD 配置、验证命令 |
| 测试策略 | test-strategy-format | 验收覆盖、测试类型映射 |
| 测试基准 | test-baseline-format | 基准线快照、趋势对比 |
...
| AI 验收验证 | 首次执行 /pace-test accept 时 | "（accept 提升审批质量——逐条附代码证据、三级判定、审查测试断言实质性、弱覆盖自动降级测试策略。不做也能过 Gate 2，做了让人类审批更高效。）" | `accept` |
