...
- 有参数 → 在 `bizdevops/backlog/` 中按标题关键词匹配
- 无参数 → 读取 `bizdevops/state.md` 的"下一步"
- 未找到对应 CR → 自动创建（格式参考 Plugin `knowledge/_schema/cr-format.md`）
- 找到被阻塞的 CR → 告知用户阻塞原因，建议替代

### Step 2：加载上下文

1. 读取 CR 文件：状态、门禁、事件记录
2. 读取 `bizdevops/rules/workflow.md`：当前阶段的准出条件
3. 读取 `bizdevops/rules/gates.md`：门禁检查项
4. 用 1 句话告知用户："从 [上次进度] 继续。"

### Step 3：自治推进

进入推进模式，自主工作。遵循 `rules/bizdevops-protocol.md` 的推进模式规则：
...
停止条件（满足任一）：
a. 所有自动门禁通过 → 到达 review_ready → 自动运行 `/pace-review` 逻辑
b. 遇到需要用户决策的技术问题 → 询问用户
c. 会话即将结束 → 保存 checkpoint

### Step 4：更新状态

1. 更新 CR 文件（门禁 checkbox、事件记录）
2. 更新 `bizdevops/state.md`（当前进度、下一步）
3. 检查关联 PF：如果所有 CR 完成 → 更新 product-line.md 追溯树
