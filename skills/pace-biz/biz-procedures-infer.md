...
输出推断摘要（新增追踪/技术债务/未实现确认数量 + 成熟度分布），然后基于推断结果动态推荐：

- 技术债务占比 > 50% → 优先推荐 `/pace-dev` 处理高优技术债务项
- 新增功能中有骨架级（就绪度 < 60%）→ 优先推荐 `/pace-biz refine [最低就绪度 ID]`
- 新增 3+ 个实体 → 追加推荐 `/pace-biz align` 检查对齐度
- 新增实体就绪度 >= 80% → 推荐 `/pace-plan next` 排入迭代
- 默认 → 列出 refine / align / /pace-dev / /pace-plan next 完整选项
