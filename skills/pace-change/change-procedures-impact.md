...
表面层的 1 句话结论按当前角色调整措辞：

| 角色 | 表面层措辞模板 |
|------|--------------|
| Biz Owner | "此变更影响 [N] 个功能，对 [OBJ] 的 MoS [指标] 可能 [正面/负面影响]。综合风险 [低/中/高]。" |
| PM | "此变更影响 [N] 个功能，预计影响迭代交付 [+/-N 天]。[M] 个 PF 需调整排期。综合风险 [低/中/高]。" |
| Dev（默认） | "这个变更影响 [N] 个功能，综合风险 [低/中/高]。[1 句话结论]"（现有格式不变） |
