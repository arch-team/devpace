...
- **降级场景（无 .devpace/）**：未初始化时仅输出即时影响评估，不创建文件（详见末尾「§降级场景」）
- **Step 0.5 Triage 分流**：变更分类（需求新增/修改/删除/优先级调整）→路由到对应流程
- **Step 1 影响分析**：BR 级→PF 级→CR 级→代码级四层影响追踪
- **Step 1.5 风险量化**：5 维风险评分（范围/技术/进度/质量/依赖）→ L/M/H 等级
- **Step 2-3 调整方案与执行**：提出方案→用户确认→执行变更→记录到变更日志
- **paused 状态规则**：CR 暂停触发条件、恢复条件和超时处理
