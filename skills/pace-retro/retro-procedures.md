...
- **行动摘要**：~10 行关键信息（指标+趋势+关注+亮点+建议+待确认），解决信息过载
- **数据收集详情**：从 CR 文件/project.md/iterations/test-baseline 提取回顾数据
- **回顾报告格式**：两层结构——行动摘要（必输出）+ 维度详情（按角色排序）
- **update 模式反馈**：输出变化摘要（哪些指标变了、变了多少）
- **风险趋势**：跨迭代风险模式/重复 Pattern/登记簿状态（`.devpace/risks/` 存在时）
- **经验沉淀规则**：从回顾中提取 pattern 写入 insights.md，含分类和置信度
- **本次学习透明段**：列出提交的 pattern、验证结果、处理状态
- **缺陷分析维度**：根因分类（6 种基础 + 项目自定义）+ 分布统计
...
- **迭代传递清单**：结构化衔接 retro→plan，写入 iterations/current.md
- **报告质量自评**：数据充分度、趋势可信度、建议可执行度三维评分
- **Agent 记忆利用**：读取上次关键发现加速定位，保存本次趋势方向
- **历史快照管理**：dashboard.md 保留跨迭代历史数据，支持趋势分析
- **验证-审批一致率深入分析**：一致率 <80% 时列出不一致 case 和原因
...
当一致率 < 80% 时，在回顾报告中额外输出不一致 case 分析：
