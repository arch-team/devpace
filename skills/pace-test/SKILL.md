---
description: Use when user says "跑测试", "测试覆盖", "验证一下", "验收", "回归", "test", "verify", "coverage", "测试策略", /pace-test, or wants to run tests, check requirement coverage, verify acceptance criteria, or manage testing strategy.
allowed-tools: AskUserQuestion, Write, Read, Edit, Glob, Grep, Bash
argument-hint: "[verify 验收验证|strategy 测试策略|coverage 覆盖分析|regress 回归分析|report 测试报告|gen 生成用例|flaky 不稳定测试|gate 模拟门禁|baseline 基准线] [CR编号|PF标题]"
model: sonnet
---
...
- `/pace-review` Gate 2：可消费 `/pace-test verify` 的验收映射报告作为审查证据
...
- `verify [CR编号]` → AI 驱动的验收验证：逐条比对 PF 验收标准与实际行为
- `strategy` → 基于 PF 验收标准生成测试策略建议
- `coverage` → 分析当前测试对 PF 验收标准的覆盖度（需求覆盖率，非代码覆盖率）
- `regress` → 回归风险分析：基于变更范围推荐需要重跑的测试
- `report` → 生成面向人类的测试摘要报告（供 Gate 3 审批参考）

**深度功能**（按需使用）：
- `gen [PF标题]` → 基于 PF 验收标准生成测试用例框架（技术栈感知）
- `flaky` → 分析历史测试结果，识别不稳定测试并建议修复/隔离
- `gate [1|2|4]` → 模拟执行指定 Gate 的完整检查流程（dry-run）
