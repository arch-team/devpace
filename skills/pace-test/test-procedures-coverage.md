...
> **职责**：/pace-test coverage 需求覆盖分析——需求覆盖率 + 代码覆盖率辅助信号。按 SKILL.md 路由表按需加载。

## §0 速查卡片

- **覆盖分析**：PF 验收条件→已有测试交叉比对，输出需求覆盖率矩阵
- **代码覆盖率**：按技术栈自动采集行/分支覆盖率（辅助信号，不替代需求覆盖率）
- **阈值检查**：test-strategy.md 配置阈值时自动对比并输出达标/告警状态
...
`覆盖：需求 X% · 代码 Y% · 未覆盖 N 条`

### 降级场景

| 条件 | 行为 |
|------|------|
| 无 test-strategy.md | 从 project.md 提取 PF 验收条件作为基准 |
| 无 project.md 功能规格 | 输出"无 PF 验收标准可分析"并终止 |
| 无代码覆盖率工具 | 跳过 Step 5 代码覆盖率采集，仅输出需求覆盖率 |
| 无阈值配置 | 跳过 Step 8 阈值检查，不输出达标/告警 |
| 无 .devpace/ | 降级模式：仅基于代码覆盖率工具输出代码覆盖率（无需求覆盖率能力） |
