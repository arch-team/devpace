...
> **职责**：/pace-test impact 变更影响分析——变更→依赖图→受影响测试。按 SKILL.md 路由表按需加载。

## §0 速查卡片

- **影响分析**：CR 代码变更→文件依赖图→受影响 PF 和测试识别
- **交叉检查**：变更热点（git 高频文件）+ flaky 历史（insights.md）→风险等级调整
- **快速执行**：`--run` 模式自动执行必跑测试并附加结果到报告
...
6. **提取回归测试建议**：
   ...
   - **flaky 交叉检查**：在 insights.md 中查找 `category: 质量保障` 且涉及受影响模块的不稳定测试记录。匹配到 → 在影响分析报告的"必跑"列表中标注"⚠️ 该模块有不稳定测试历史（[pattern 摘要]），建议优先验证"，并将对应 PF 的风险评级上调一级（🟢→🟡 或 🟡→🔴）
7. **变更频率分析**（Git 历史辅助信号）：
   - 执行 `git log --since="3 months ago" --name-only --pretty=format:""` 统计每个文件的变更次数
   - 识别高频变更文件（3 个月内变更 ≥5 次的文件）
   - 将高频变更文件与本次变更文件交叉比对：
     - 本次变更的文件同时是高频变更文件 → 标注"🔥 高频变更模块——历史变更多+本次再变更 = 更高回归风险"
     - 在影响分析报告的"风险评估"section 追加"变更热点"子段
   - 无高频变更文件 → 静默跳过
   ...
   **输出格式**（追加到风险评估段）：
   ...
   | 文件 | 近 3 月变更次数 | 本次变更 | 风险提升 |
   |------|---------------|---------|---------|
   | src/auth.py | 8 次 | ✅ 是 | 🔥 高 |
   | src/utils.py | 6 次 | ✅ 是 | 🔥 高 |
   ```
8. **评估风险等级**：基于变更范围大小 × PF 重要性（优先级）综合评定
9. **快速执行模式**（`impact --run`）：
...
| 受影响模块有不稳定测试历史（insights.md 匹配） | 风险等级上调一级 |
