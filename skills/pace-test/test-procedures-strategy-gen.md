...
> **职责**：/pace-test strategy 测试策略生成——PF→测试类型映射、金字塔分析。按 SKILL.md 路由表按需加载。

## §0 速查卡片

- **策略生成**：PF 验收标准→测试类型映射（主类型+辅助类型），输出测试矩阵
- **金字塔分析**：unit/integration/E2E/manual 占比评估 + 健康度判定
- **非功能推荐**：识别 security/performance 辅助类型→推荐 checks.md 检查项
...
0. **陈旧检测**：
   - 如果 `.devpace/rules/test-strategy.md` 已存在 → 检查文件是否包含陈旧标记 `<!-- STALE:`
   - 有陈旧标记 → 输出"⚠️ 当前策略已过期（[标记内容]）。将基于最新 PF 验收标准重新生成。"→ 继续执行后续步骤（重新生成覆盖旧文件）
   - 无陈旧标记 → 输出"当前策略存在且未过期。确认重新生成（覆盖现有策略）还是跳过？"→ 用户确认后继续或终止
   - 不存在 → 正常执行（首次生成）
1. **提取 PF 验收标准**：
...
2. **检测技术栈**：按 test-procedures-common.md「技术栈检测（SSOT）」
...
9. **非功能性测试 checks 推荐**（当识别到辅助类型时）：
   ...
   - 推荐模板根据 test-procedures-common.md「技术栈检测（SSOT）」检测到的技术栈自适应选择工具
...
12. **测试实施指导**：为策略中出现的每种测试类型生成可执行的实施指导：
   ...
   **生成规则**：
   ...
   - 优先匹配 test-procedures-common.md「技术栈检测（SSOT）」检测到的技术栈；多技术栈时全部列出
   - 已有测试框架时（context.md 中声明或自动检测到），推荐与已有框架一致的方案
   - 项目已有同类测试配置时（如已有 `jest.config.js`），标注"已配置"并跳过初始化步骤

### 输出分层（遵循 SKILL.md 分层输出约定）

strategy 子命令的三级输出：

**摘要层（--brief）**：
```
策略：N PF · M 条验收 · 覆盖 X% · 金字塔 [状态] · 待建 K 条
```
...
- 测试金字塔健康度表
- 后续引导（1 行）
- 非功能性测试 checks 推荐（如有辅助类型）

**详细层（--detail）**：
标准层全部内容 + 以下扩展：
- 逐 PF 验收条件→测试映射表（Step 4 完整输出）
- 测试数据策略（Step 11）
- 测试实施指导（Step 12）

**规则**：
- 默认标准层的输出量控制在 ~30 行以内（降低认知负荷）
- Step 4 的映射表在标准层仅输出未覆盖条目（`❌ 待建` + `⚠️ 部分覆盖`），完整映射移至 --detail
- Step 11/12 的详细内容仅在 --detail 层输出
...
其他子命令（accept、coverage、impact、report）读取 test-strategy.md 时，如检测到 `<!-- STALE:` 标记：
- 在输出中追加 1 行提示："⚠️ 测试策略已过期，建议执行 `/pace-test strategy` 刷新。当前分析基于过期策略，结果可能不完整。"
- 不阻断当前子命令执行——使用过期策略继续，仅提醒
