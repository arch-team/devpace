...
> **职责**：/pace-test 的详细执行规则——基础测试执行（§1）、测试用例生成（§2）、测试策略生成（§3）、需求覆盖分析（§4）、回归风险分析（§5）、测试摘要报告（§6）、不稳定测试分析（§7）、模拟门禁执行（§8）、测试基准线（§9）。
...
当用户执行 `/pace-test gate [1|2|4]` 时，以 dry-run 模式执行指定 Gate 的完整检查流程，不触发状态转换。
...
| 维度 | /pace-test gate | /pace-dev Gate |
|------|----------------|----------------|
...
*提示：修复后可再次执行 `/pace-test gate [N]` 确认，或执行 `/pace-dev` 进入正式 Gate 流程。*
```
...
| 无活跃 CR 且无参数 | 提示"请指定 Gate 编号：/pace-test gate 1" |
